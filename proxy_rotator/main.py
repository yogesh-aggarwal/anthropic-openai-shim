...
class ProxyManager:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    PROXY_LIST_URL, timeout=aiohttp.ClientTimeout(total=30)
                ) as resp:
                    if resp.status == 200:
            ...
            for u in new_file_urls:
                incoming_urls[u] = "file"
            for u in new_url_urls:
                if u not in incoming_urls:
                    incoming_urls[u] = "url"
            ...
            if incoming_urls:
                ...
                logger.info(
                    f"Proxy Pool Updated: {file_count} Priority (File), {url_count} Secondary (URL)"
                )
            else:
    ...
    def get_proxy(self, auth_header: str, attempt: int = 0) -> str | None:
        if not self.proxies:
            return None
